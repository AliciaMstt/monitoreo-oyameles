...
```{r}
# check if there are any trees with healthy + other. If this is the case, there should be an " " after healthy. Only non-approved records should appear here:
muestreo[grep("healthy ", muestreo$tree_health), c("plot", "tree_id", "X_validation_status")]

# remove any tree labeled with healthy + something else, independently of its validation status
...
The previous code checked for capture errors and re-formated the data accordingly. The validation of each record in terms of the type damage meeting the picture was done manually in the web app. Downstream analyses filter data in order to only perform analyses with approved records. The distribution of approved and not approved records is as follows:

```{r}
table(muestreo$X_validation_status)
```
...
```{r}
#Make muestreo within validation aproved
muestreo2<- filter(muestreo, X_validation_status == "validation_status_approved")


#Make summary 
plots_health <- group_by(muestreo2, plot) %>% 
