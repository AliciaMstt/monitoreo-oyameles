...
summary_by_ref<- refo_expo %>%  
  ...
  summarise(n = n()) %>%
  # create percentage within each plot
  mutate(perc = n / sum(n)) %>%
  # normalize it
  mutate(perc.damage.log = log(n / sum(n)))

# the percentage of healthy and damaged trees for each reforested yes/no category is shown. The sum of healthy/damaged by ref category sums 1.
head(summary_by_ref) 
...
head(summary_by_ref) # the percentage of damaged trees by each reforestation category is shown
...
# the percentage of healthy and damaged trees for each exposed covered/exposed category is shown. The sum of healthy/damaged by category sums 1.
head(summary_by_exp) 
...
head(summary_by_exp) # the percentage of damaged trees by each reforestation category is shown

# The percentages dont sum 1 because we are only looking at the % of damaged trees by category, NOT to the % of healthy trees.
...
library("gridExtra")
library("cowplot")
plot_grid(p_3a, p_3c, ncol = 2, nrow = 1)
...
NRp_3c<-ggplot(cont_tab_3, aes(tree_exposition, ..count..)) +
  ...
  scale_fill_manual(name ="Health status", 
                    ...
                    labels= c("healthy", "ozone","ozone and other"))+
  theme_bw()+ 
  ggtitle("b)")+theme(legend.title.align = 0.5)+theme(text = element_text(size = 20))+ 
  theme(plot.title = element_text(lineheight=1.1, face="plain"))+
  labs(y="Number of trees", x= "Tree exposition")
...
library("gridExtra")
library("cowplot")
plot_grid(Rp_3a, NRp_3c, ncol = 2, nrow = 1)
