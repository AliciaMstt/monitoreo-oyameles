...
p4_c<-condition_HOO%>%
   ggplot(aes(y= sqrt(tree_nodes), x= tree_health_simplified))+
...
condition_HOO%>%
  ggplot(aes(x = sqrt(tree_nodes)))+
  geom_histogram(aes(y= ..density..),
                 binwidth = 0.1, color = "black", fill = "cyan3")+
  xlab("Nodos en los árboles")+ ylab("Frecuencia")+
  ggtitle("Histograma de los nodos de los árboles")+
  stat_function(fun = dnorm, args = list(mean = mean(sqrt(muestreo_tidy$tree_nodes)), 
                                         sd = sd(sqrt(muestreo_tidy$tree_nodes))), color= "deepskyblue4", size= 1)
...
# Homogeneidad de varianzas. No hay en valores menores a 0.05
library(car)
leveneTest(sqrt(tree_nodes) ~ tree_health_simplified, data = condition_HOO)


pairwise.t.test(sqrt(condition_HOO$tree_nodes), condition_HOO$tree_health_simplified,
...
condition_PO<-muestreo_tidy%>%
  filter(ozone_damage_percentage == "less than 10%" | ozone_damage_percentage == "10 to 40%" | ozone_damage_percentage == "40 to 50%"| ozone_damage_percentage == "50 to 70%" | ozone_damage_percentage == "more than 70%")
...
condition_PO$ozone_damage_percentage
p4_d<-condition_PO%>%
   ggplot(aes(y= sqrt(tree_nodes), x= ozone_damage_percentage))+
          geom_boxplot(color="grey", notch = F)+
...
# Homogeneidad de varianzas. No hay en valores menores a 0.05. Debe dar valores mayores a 0.05 para que haya homogeneidad
leveneTest(sqrt(tree_nodes) ~ ozone_damage_percentage, data = condition_PO)


pairwise.t.test(sqrt(condition_PO$tree_nodes), condition_PO$ozone_damage_percentage,
                 p.adjust.method = "BH", pool.sd = FALSE)

# Normalidad. No hay en valores menores a 0.05.Debe dar valores mayores a 0.05 para que haya normalidad
...
# Graficar modelo 3
Tree_height_plot<-ggplot(condition_HOO, aes(x = tree_nodes, y = log(tree_heigth))) + geom_point(aes(colour= 
tree_health_simplified)) + geom_smooth(method="glm",aes(color= tree_health_simplified), fullrange =T)+
  scale_color_manual(values= my_cols)+
  theme_bw()

print(Tree_height_plot + labs( y = "log(Higth tree)", x = "Number of nodes") + labs(color = "Status of healthy"))
...
p + theme_bw() +
  labs(x="Altitude of plots", 
       y= "Tree percentage ozone damage trees")+
       theme(plot.title = element_text(lineheight=1.1, face="bold")) +
