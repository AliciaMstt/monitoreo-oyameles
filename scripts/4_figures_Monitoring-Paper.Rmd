...
## subset of data only with reforestatiton, health and exposition
...
### Summarise and estimate n and % by category

## For Health
summary_by_health<- refo_expo %>%  
           group_by(plot, block, tree_health_simplified) %>% 
            # count how many individuals there are in each category
           summarise(n.health = n()) %>%
           # create percentage within each plot (summing all rows of each plot = 1)
           mutate(perc.health = n.health / sum(n.health)) 

## By reforestation
summary_by_ref<- refo_expo %>%  
  group_by(plot, block, reforested) %>% 
  # count how many individuals there are in each category
  summarise(n.ref = n()) %>%
  # create percentage within each plot (summing all rows of each plot = 1)
  mutate(perc.ref = n.ref / sum(n.ref))

## By exposition
summary_by_exp<- refo_expo %>%  
  group_by(plot, block, tree_exposition) %>% 
  # count how many individuals there are in each category
  summarise(n.exp = n()) %>%
  # create percentage within each plot (summing all rows of each plot = 1)
  mutate(perc.exp = n.exp / sum(n.exp))

## Join 

# Health and reforestation
health_by_ref<-left_join(summary_by_health, summary_by_ref, 
                          by=c("plot"= "plot", "block"="block"))

## Health and exposure
health_by_exp<-left_join(summary_by_health, summary_by_exp, 
                         by=c("plot"= "plot", "block"="block"))

### Next: check tydiverse examples, we still dont have the variables we need. 
...
########################
library(lme4)
model<- glmer(tree_heigth ~ . + (1|plot), data = condition_model_all_C, family = Gamma(link = "log"))
summary(model)
...
glm_1_coh<-glmer(tree_heigth ~ tree_nodes*tree_health_simplified*tree_exposition * reforested * tree_exposition + (1|plot), data = condition_model_all_C, family = Gamma(link = "log"))
summary(glm_1_coh)
#### PLOT MODEL
par(mfrow =c(2,2))
plot(glm_1_coh)
...
########################
glm_7_coh<-glmer(tree_heigth ~ tree_nodes*tree_health_simplified*reforested + tree_exposition + (1|plot), data = condition_model_all_C, family = Gamma(link = "log"))
summary(glm_7_coh)
#### PLOT MODEL
par(mfrow =c(2,2))
plot(glm_7_coh)
...
########################
glm_8_coh<-glmer(tree_heigth ~ tree_nodes*tree_health_simplified*tree_exposition+reforested + (1|plot), data = condition_model_all_C, family = Gamma(link = "log"))
summary(glm_8_coh)
#### PLOT MODEL
par(mfrow =c(2,2))
plot(glm_8_coh)
...
########################
glm_9_coh<-glmer(tree_heigth ~ tree_nodes*tree_health_simplified*(tree_exposition+reforested) + (1|plot), data = condition_model_all_C, family = Gamma(link = "log"))
summary(glm_9_coh)
#### PLOT MODEL
par(mfrow =c(2,2))
plot(glm_9_coh)
