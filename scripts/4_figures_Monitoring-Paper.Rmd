...
muestreo_tidy<-muestreo_tidy%>% mutate(Bloque = case_when(plot %in% c(1,2,3) ~1,
                 plot %in% c(1,2,3) ~1,
                 plot %in% c(4,5,6) ~2,
                 plot %in% c(7,8,9) ~3,
...
```{r, fig.height=6, fig.width=6, message=FALSE}
###################################
condition_data<-dplyr::select(muestreo_tidy, contains(c("plot", "bloque","tree_health_simplified", "reforested")))%>%
  filter(tree_health_simplified == "healthy"| tree_health_simplified == "ozone" |tree_health_simplified == "ozone_and_other")
#0 es no y 1 en yes
...
oo_per_natreg<-condition_data%>%
  ...
  group_by(plot) %>% 
  count(plot)
oo_per$natreg<- oo_per_natreg$n
oo_per$total<- oo_per_natreg+oo_per$reforested
