...
lm3<-lm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified + tree_exposition + reforested, data = condition_HOO)

# Ho= hay normalidad, si pvalues es mayor a 0.05 se acepta la Ho por lo tanto pvalue mayor a 0.05 hay normalidad 
shapiro.test(lm3$residuals) # Normalidad ( Datos normales si es mayor a 0.05)

cor.test(abs(lm3$residuals), lm3$fitted.values) #Homocedasticidad: Valor no significativa sig que si hay homocedasticidad
...
tree_health_simplified)) + geom_smooth(method="lm",aes(color= tree_health_simplified), fullrange =T)+
  scale_color_manual(values= c("#b9aa46","#a361c7", "#6ab64c", "#c65c8a", "#4fb79b", "#cb4f42", "#6587cd", "#c78143", "#607a38"))
