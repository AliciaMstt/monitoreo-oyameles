...
# Modelos de acuerdo a Altura 

## Modelo 1 lm(log10(tree_heigth) ~ tree_nodes, data = condition_HOO)

```{r modelo 1, echo= TRUE}

# Modelo 1 - Solo edad afecta crecimiento
glm1<-glm(log10(tree_heigth) ~ tree_nodes, data = condition_HOO)

# Ho= hay normalidad, si pvalues es mayor a 0.05 se acepta la Ho por lo tanto pvalue mayor a 0.05 hay normalidad 
shapiro.test(glm1$residuals) # Normalidad ( Datos normales si es mayor a 0.05)
cor.test(abs(glm1$residuals), glm1$fitted.values) # Homocedasticidad: Valor no significativa sig que si hay homocedasticidad
...
## Modelo 2 lm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified, data = condition_HOO)

```{r modelo 2, echo= TRUE}
# Modelo 2  - Edad y salud afectan crecimiento
glm2<-lm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified, data = condition_HOO)

# Ho= hay normalidad, si pvalues es mayor a 0.05 se acepta la Ho por lo tanto pvalue mayor a 0.05 hay normalidad 
shapiro.test(glm2$residuals) # Normalidad ( Datos normales si es mayor a 0.05)
cor.test(abs(glm2$residuals), glm2$fitted.values) #Homocedasticidad: Valor no significativa sig que si hay homocedasticidad
...
## Modelo 3 lm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified + tree_exposition , data = condition_HOO)

```{r modelo 3, echo= TRUE}
# Modelo 3  - Edad, salud y estructura espacial afectan crecimiento
glm3<-glm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified+tree_exposition, data = condition_HOO)
...
## Modelo 4 lm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified + reforested, data = condition_HOO)

```{r modelo 4, echo= TRUE}
# Modelo 4  - Edad, salud y estructura espacial afectan crecimiento
glm4<-glm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified + reforested, data = condition_HOO)

# Ho= hay normalidad, si pvalues es mayor a 0.05 se acepta la Ho por lo tanto pvalue mayor a 0.05 hay normalidad 
shapiro.test(glm4$residuals) # Normalidad ( Datos normales si es mayor a 0.05)
cor.test(abs(glm4$residuals), glm4$fitted.values) #Homocedasticidad: Valor no significativa sig que si hay homocedasticidad
...
## Modelo 5 lm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified + reforested + tree_exposition, data = condition_HOO)

```{r modelo 5, echo= TRUE}
# Modelo 5  - Edad, salud y estructura espacial afectan crecimiento
glm5<-glm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified + reforested + tree_exposition, data = condition_HOO)

# Ho= hay normalidad, si pvalues es mayor a 0.05 se acepta la Ho por lo tanto pvalue mayor a 0.05 hay normalidad 
shapiro.test(glm5$residuals) # Normalidad ( Datos normales si es mayor a 0.05)
cor.test(abs(glm5$residuals), glm5$fitted.values) #Homocedasticidad: Valor no significativa sig que si hay homocedasticidad
...
cor.test(abs(glm5$residuals), glm3$fitted.values) #Homocedasticidad: Valor no significativa sig que si hay homocedasticidad
...
parcelas_tidy_46<-parcelas_tidy %>% 
  filter( X_coordinates_altitude>2800)


# Create new variable with percentage of ozone damage
parcelas_tidy_46<-parcelas_tidy_46 %>% rowwise() %>% 
                     mutate(., 
                      total=sum(healthy,ozone,ozone_and_other,
                          drougth, acid_rain, other,
                          others_combined, dead, fungi,
...
#Statistical
leveneTest( parcelas_tidy_46$X_coordinates_altitude, parcelas_tidy_46$perc.ozone)

#plot
p <- ggplot(parcelas_tidy_46) +
     geom_point(aes(x=X_coordinates_altitude,
             y=perc.ozone))

p<- ggplot(parcelas_tidy_46, aes(X_coordinates_altitude, perc.ozone ))+
  geom_point(color= "grey50", size = 3, alpha = 0.6)
...
# Figure 5 without ozone + others

# Modelos de acuerdo a Altura 

## Modelo 1 lm(log10(tree_heigth) ~ tree_nodes, data = condition_HOO)

```{r modelo 1 without ozone + others, echo= TRUE}

# Modelo 1 - Solo edad afecta crecimiento
glm1<-glm(log10(tree_heigth) ~ tree_nodes, data = condition_HOz)
...
```{r modelo 2 without ozone + others, echo= TRUE}
# Modelo 2  - Edad y salud afectan crecimiento
glm2<-lm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified, data = condition_HOz)
...
```{r modelo 3 without ozone + others, echo= TRUE}
# Modelo 3  - Edad, salud y estructura espacial afectan crecimiento
glm3<-glm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified+tree_exposition, data = condition_HOz)
...
```{r modelo 4 without ozone + others, echo= TRUE}
# Modelo 4  - Edad, salud y estructura espacial afectan crecimiento
glm4<-lm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified + reforested, data = condition_HOz)
...
```{r modelo 5 without ozone + others, echo= TRUE}
# Modelo 5  - Edad, salud y estructura espacial afectan crecimiento
glm5<-glm(log10(tree_heigth) ~ tree_nodes*tree_health_simplified + reforested + tree_exposition, data = condition_HOz)
...
```{r Comparar modelos without ozone + others, echo= TRUE}
AIC(glm1,glm2, glm3, glm4, glm5)
```
...
# Graficar modelo 4
Tree_height_plot<-ggplot(condition_HOz, aes(x = tree_nodes, y = log(tree_heigth))) + geom_point(aes(colour=tree_health_simplified), alpha=0.5, size= 2.5) + geom_smooth(method="glm",aes(color= tree_health_simplified), fullrange =T)+
  labs( y = "log(Tree higth)", x = "Tree age (years)", color = "Health status")+
  scale_color_manual(values=  my_cols, labels= desired_names,
                    name= "Health status")+
  theme(text = element_text(size = 20), axis.text.x=element_blank())+
